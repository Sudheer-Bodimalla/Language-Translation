...
def main():
    ...
    if st.sidebar.button("Take a Quiz"):
        quiz_word = translator.translate("Hello", dest='hi').text
        user_answer = st.sidebar.text_input(f"What does '{quiz_word}' mean in English?")
        if user_answer.lower() == "hello":
    ...
    if st.sidebar.button("Get Daily Phrase"):
        daily_phrase = "The early bird catches the worm"
        translated_phrase = translator.translate(daily_phrase, dest='hi').text
        st.sidebar.write(f"English: {daily_phrase}")
        st.sidebar.write(f"Hindi: {translated_phrase}")
